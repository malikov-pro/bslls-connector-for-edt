...
public class BSLValidator implements IExternalBslValidator {
	...
	var code = diagnostic.getCode().map(c -> c, Object::toString);
	if (severity == DiagnosticSeverity.Error) {
	    messageAcceptor.acceptError(message, module, offsetParams[0], offsetParams[1], code);
	} else if (severity == DiagnosticSeverity.Warning) {
	    messageAcceptor.acceptWarning(message, module, offsetParams[0], offsetParams[1], code);
	} else {
	    messageAcceptor.acceptInfo(message, module, offsetParams[0], offsetParams[1], code);
	}
