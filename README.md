...
1. Откройте `Окно` → `Параметры` → `Коннектор BSL LS`. Флажок **«Включить плагин»** (включён по умолчанию) полностью управляет работой: в выключенном состоянии процесс BSL LS не запускается и замечания не создаются; изменение применяется по **«Применить»/«OK»**.
