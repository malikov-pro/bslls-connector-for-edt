...
1. Откройте `Окно` → `Параметры` → `Коннектор BSL LS`. Флажок **«Включить плагин»** (включён по умолчанию) полностью управляет работой: в выключенном состоянии процесс BSL LS не запускается, замечания не создаются, а уже опубликованные замечания BSL LS снимаются; изменение применяется по **«Применить»/«OK»**.
...
4. Если BSL LS не успевает стартовать на крупной конфигурации, увеличьте **«Таймаут initialize, с»** (умолчание 60, диапазон 5–3600). Признак нехватки: в журнале «BSL LS не ответил на initialize за N с».
5. Замечания появляются сами — проверки BSL LS включаются автоматически при установке. Если их нет, убедитесь, что проверки не выключены:
...
6. **Применить** — плагин перезапустит подключение к LS.
7. Проверка: сохраните BSL-модуль с нарушением (например, строку длиннее 120 символов) — в панели `Ошибки конфигурации` появится замечание с префиксом `[BSL LS]`. Если замечаний нет — см. [issue #2](https://github.com/malikov-pro/bslls-connector-for-edt/issues/2) и журнал ошибок (`Окно` → `Журнал ошибок`); stderr процесса LS пишется в `~/.bsl-connector-for-edt/logs/ls-stderr-<режим>.log`.

Если поднять BSL LS не удалось, коннектор не повторяет попытку на каждом модуле (иначе каждая проверка оплачивала бы таймаут initialize): отказ запоминается, в журнал пишется одна запись. Повторная попытка — после **«Проверить»** или **«Применить»** в настройках коннектора либо после перезапуска EDT.

Процесс BSL LS стартует, только если хотя бы в одном проекте воркспейса включена категория «Проверка BSL LS» в профиле валидации проекта (Свойства проекта → Валидация); для проектов с выключенной категорией модули не валидируются и LS не будится. При включении проверок в свойствах проекта LS поднимается автоматически при следующей проверке.
...
(поиск идёт и по подпапкам, берётся первый найденный). Если файла нет, коннектор
генерирует свой конфиг `~/.bsl-connector-for-edt/generated-config.json`:
`sendErrors: never` (см. ниже) и `configurationRoot` единственного проекта
воркспейса. Файл, созданный вручную, всегда имеет приоритет.
Кнопка **«Открыть рабочую папку»** на странице настроек открывает
`~/.bsl-connector-for-edt` (слоты скачанного BSL LS и `logs/ls-stderr-<режим>.log`).
Сам конфиг `.bsl-language-server.json` кладите не туда, а в корень воркспейса EDT — он находится автоматически.

Файл должен содержать:
* `sendErrors: never` — при значении по умолчанию (`ask`) BSL LS во время
  диагностики спрашивает клиента «отправить ошибку разработчикам?» и ждёт
  ответа без таймаута; у headless-клиента коннектора ответа нет, и поток
  диагностики зависает (проявлялось как вечное «Расширенная проверка модулей», [#14]);
* путь к метаданным проекта в свойстве `configurationRoot` — без него LS не
  знает типы модулей, и часть диагностик не работает или даёт ложные срабатывания ([#4]).

⚠️ Не задавайте `computeTrigger: onSave`: коннектор запрашивает диагностики
pull-запросами для несохранённого текста, и LS в этом режиме отвечает
устаревшим (пустым) результатом — ломаются «Подавить» и замечания по
несохранённым модулям.
...
* Проверки BSL LS работают при проверках в IDE и при «Проверке конфигурации»
  в EDT. Запуск валидации внешними средствами (`ring EDT Validate`, headless
  валидация) вызывается в отдельном процессе без нашего подключённого BSL LS —
  замечания `[BSL LS]` в таких результатах не появятся ([#5]).
